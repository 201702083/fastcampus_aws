...
Chapter 2-4. (4)Jira API 소개 및 Postman 활용하기

지라는 자체적으로 다양한 API 들을 지원합니다. 그리고 동시에 이런 Jira API 들을 테스트 하기 위해서는 무료툴인 Postman을 잘 활용하면 좋습니다.
이번 강의에서는 그런 Jira API 사이트와 그리고 API의 간단한 기능들을 소개하고, Postman 활용법을 소개합니다. 

Jira API 사이트 : 
https://docs.atlassian.com/software/jira/docs/api/REST/8.4.2/?_ga=2.93052573.1639889686.1571238783-647032862.1567516016
...
존재하는 키여야 해당 내용을 불러 올 수 있습니다. 


위와 같은 내용들을 실행해보기 위해서는 지라를 설치한 경로를 정확히 알아야 합니다.
그리고 지라를 실행한 뒤에 

API URL : (지라 설치 경로)/rest/api/2/issue/{issueIdOrKey}
형태로 URL로서 해당 경로에 접속해서 API가 활성화 되어있는지 확인해주어야 합니다. 
